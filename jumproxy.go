...
func parseInput(input string) (net.IP, error) {
	...
	// If not an IP address, try to resolve as a hostname
	ips, err := net.LookupIP(input)
	if err != nil {
		return nil, fmt.Errorf("failed to resolve hostname or parse IP: %v", err)
	}
...
func handleinputs() {
	...
	filename = flag.String("k", "", "The ASCII text passphrase")
	flag.Parse()
